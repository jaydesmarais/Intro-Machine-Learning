...
class NN:
    ...
    def compute_gradients(self, X, Y):
        ...
        for idx in range(num_layers-2, -1, -1):
            ...
            ### YOUR CODE HERE ###
            E = np.matmul(ws[idx], D_stack[idx]) + bs[idx]
            backprop_grad = self.activation_function.backprop_grad(E)
            grad = backprop_grad * (np.matmul(ws[idx+1].T, grad))
